      ...
      {index > 0 && (
        <div className="flex items-center gap-3 py-1">
          ...
          <select
            ...
            onChange={(e) =>
              onChange({ ...value, logicOperator: e.target.value as "AND" | "OR" })
            }
        ...
        <div className="grid grid-cols-3 gap-2 flex-1">
          <div>
            <select
              ...
              onChange={(e) =>
                onChange({ ...value, indicator: e.target.value as RuleFormValue["indicator"] })
              }
          ...
          <div>
            <select
              ...
              onChange={(e) =>
                onChange({ ...value, condition: e.target.value as RuleFormValue["condition"] })
              }
  ...
  return (
    <div className="space-y-1">
      ...
      <p className="text-[12px] text-slate-600 pt-2">
        {ruleType} rule: bot {ruleType === "Entry" ? "opens" : "closes"} a trade when ALL/ANY
        conditions are met.
      </p>
    </div>
  );
...
function EditForm({ strategy }: { strategy: StrategyDetail }) {
  ...
  const entryRules = strategy.rules_rel
    ...
    .map((r) => ({
      indicator: r.indicator as RuleFormValue["indicator"],
      condition: r.condition as RuleFormValue["condition"],
      value: r.value,
      logicOperator: r.logicOperator as RuleFormValue["logicOperator"],
    }));
  ...
  const form = useForm<StrategyFormValues>({
    ...
    defaultValues: {
      name: strategy.name,
      pair: strategy.pair as StrategyFormValues["pair"],
      timeframe: strategy.timeframe as StrategyFormValues["timeframe"],
      entryRules: entryRules.length > 0 ? entryRules : [{ indicator: "RSI", condition: "GREATER_THAN", value: 50, logicOperator: "AND" }],
      exitRules: exitRules.length > 0 ? exitRules : [{ indicator: "RSI", condition: "LESS_THAN", value: 30, logicOperator: "AND" }],
    },
  });
  ...
  const entryFieldArray = useFieldArray({ control: form.control, name: "entryRules" });
  const exitFieldArray = useFieldArray({ control: form.control, name: "exitRules" });
  const values = watch();
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {step === 1 && (
            <div>
              ...
              <RulesSection
                fields={entryFieldArray.fields.map((f, i) => ({
                  ...f,
                  ...values.entryRules[i],
                }))}
                errors={
                  errors.entryRules as Record<
                    number,
                    {
                      indicator?: { message?: string };
                      condition?: { message?: string };
                ...
                onRemove={(i) => entryFieldArray.remove(i)}
                ruleType="Entry"
              />
              {errors.entryRules?.root && (
                <p className="mt-2 text-[12px] text-loss-400">
                  {errors.entryRules.root.message}
                </p>
              )}
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              ...
              <RulesSection
                fields={exitFieldArray.fields.map((f, i) => ({
                  ...f,
                  ...values.exitRules[i],
                }))}
                errors={
                  errors.exitRules as Record<
                    number,
                    {
                      indicator?: { message?: string };
                      condition?: { message?: string };
                ...
                onRemove={(i) => exitFieldArray.remove(i)}
                ruleType="Exit"
              />
              {errors.exitRules?.root && (
                <p className="mt-2 text-[12px] text-loss-400">
                  {errors.exitRules.root.message}
                </p>
              )}
            </div>
          )}
...
export default function EditStrategyPage() {
  ...
  const breadcrumbs = [
    ...
    ...(strategy ? [{ label: strategy.name, href: `/dashboard/strategies/${id}` }] : []),
    { label: "Edit" },
  ];
  ...
  return (
    <div>
      <PageHeader
        title={strategy ? `Edit: ${strategy.name}` : "Edit Strategy"}
      ...
      {strategy && <EditForm strategy={strategy} />}
    </div>
  );
}
